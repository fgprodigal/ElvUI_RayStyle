	...
	if(not Buttons:IsShown()) then
		if(not specButtons) then
			...
			for index = 1, numSpecs do
				...
				SpecButton:CreateBackdrop("Default")
				SpecButton.backdrop:SetAllPoints()
			end
...
local function HandleClick(self)
	...
	if(collapsed) then
		if(PositionDownwards()) then
			Handle.Arrow:SetTexture("Interface\\AddOns\\ElvUI_RayStyle\\media\\arrow-down-active")
			Handle:SetPoint("TOP", BonusRollFrame, "BOTTOM", 0, 2)
		else
			Handle.Arrow:SetTexture("Interface\\AddOns\\ElvUI_RayStyle\\media\\arrow-up-active")
...
local function ItemButtonEnter(self)
	...
	if PositionDownwards() then
		GameTooltip:Point("TOPLEFT", Container, "BOTTOMLEFT", 0, -2)
	else
...
function Container:PLAYER_LOGIN()
	...
	Thumb.bg:SetTemplate("Default", true)
	...
	uptex:SetTexture("Interface\\AddOns\\ElvUI_RayStyle\\media\\arrow-up-active")
	uptex:Size(8, 8)
	...
	uptex:SetTexture("Interface\\AddOns\\ElvUI_RayStyle\\media\\arrow-down-active")
	uptex:Size(8, 8)
	uptex:SetPoint("CENTER")
	uptex:SetVertexColor(1, 1, 1)
	S:HandleButton(Down)
