...
function watcherPrototype:OnEvent(event, unit)
	if event == "PLAYER_ENTERING_WORLD" then
		...
		E:CreateMover(self.holder, self.name.."Holder", self.name, true)
...
function RW:Initialize()
	...
	if type(E.RS["Watcher"]["filters"][E.myclass]) == "table" then
		for _, t in ipairs(E.RS["Watcher"]["filters"][E.myclass]) do
			self:NewWatcher(t)
		end
	end
	if type(E.RS["Watcher"]["filters"]["ALL"]) == "table" then
		for _, t in ipairs(E.RS["Watcher"]["filters"]["ALL"]) do
			self:NewWatcher(t)
		end
	end
	wipe(E.RS["Watcher"]["filters"])
