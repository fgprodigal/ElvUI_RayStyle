...
local RM = E:NewModule("Reminder", "AceTimer-3.0")
RM.CreatedReminders = {}
...
function RM:ReminderIcon_OnUpdate(elapsed)
	...
	if(self.elapsed and self.elapsed > 0.2) then
		local db = P.RS["Reminder"].filters[E.myclass][self.groupName]
...
function RM:FilterCheck(frame, isReverse)
	...
	local db = P.RS["Reminder"].filters[E.myclass][frame.groupName]
...
function RM:CreateReminder(name, index)
	...
	frame:Point("BOTTOM", ElvUF_Player, "TOP", 0, 130)
	E:CreateMover(frame, "ReminderMover", L["重要buff提醒锚点"], true, nil)
...
function RM:CheckForNewReminders()
	local db = P.RS["Reminder"].filters[E.myclass]
