...
function RS:Dump(text)
    text = text:trim():match("^(.-);*$")
    local f, err = loadstring("unpack(ElvUI):GetModule('RayStyle'):PrintLiteral(" .. text .. ")")
    if not f then
        E:Print("|cffff0000Error:|r", err)
    else
        f()
    end
end

function RS:EnableDumpTool()
	RS:RegisterChatCommand("dump", "Dump")
end
hooksecurefunc(CH, "Initialize", RS.EnableDumpTool)